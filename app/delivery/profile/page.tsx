...
export default function DeliveryProfilePage() {
    ...
    const profileInputRef = useRef<HTMLInputElement>(null); // For profile pic
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const newImages = [...(profile.licenseImage || []), url];
    ...
    return (
        <div className="pb-24 md:pb-8 bg-gray-900 min-h-full">
            ...
            <div className="bg-gray-800 p-8 pb-8 rounded-b-[2.5rem] shadow-lg border-b border-gray-700 relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-32 bg-linear-to-b from-green-900/20 to-transparent pointer-events-none" />
                
                <div className="flex flex-col items-center text-center relative z-10">
                    {/* Profile Picture with Click-to-Edit */}
                    <div 
                        ...
                        >
                            ...
                            <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs text-gray-500 uppercase font-bold block mb-1">Vehicle Model</label>
                                        {isEditing ? (
                                            <input 
                                            ...
                                            <p className="text-gray-200 font-medium">{profile.vehicleName}</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-500 uppercase font-bold block mb-1">Vehicle Number</label>
                                        {isEditing ? (
                                            <input 
                                                value={formData.vehicleNo}
                                                onChange={e => setFormData({...formData, vehicleNo: e.target.value})}
                                                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-green-500 outline-none uppercase"
                                            />
                                        ) : (
                                            <p className="text-gray-200 font-medium">{profile.vehicleNo}</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-500 uppercase font-bold block mb-1">Owner Name</label>
                                        {isEditing ? (
                                            <input 
                                                value={formData.vechicleOwnerName}
                                                onChange={e => setFormData({...formData, vechicleOwnerName: e.target.value})}
                                                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-green-500 outline-none"
                                            />
                                        ) : (
                                            <p className="text-gray-200 font-medium">{profile.vechicleOwnerName}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700 text-center">
                                    <h4 className="text-gray-400 text-xs font-bold uppercase">Completed</h4>
