...
export default function AdminShopsPage() {
  ...
  const deriveShopData = (shop: any) => {
    const totalEarning = shop.totalEarning ?? shop.metrics?.totalEarnings ?? 0;
    const totalOrders = shop.totalOrders ?? shop.metrics?.totalOrders ?? 0;
    const activeOrders = shop.activeOrders ?? shop.metrics?.totalActiveOrders ?? 0;
    const totalCustomers = shop.totalCustomers ?? shop.metrics?.totalCustomers ?? 0;

    return {
      name: shop.shopName ?? shop.name ?? "Unknown Shop",
      location: shop.location ?? shop.address ?? "Location not set",
      phone: shop.user?.phone ?? shop.contact?.phone ?? "N/A",
      rating: shop.averageRating ?? shop.rating ?? 0,
      status: shop.status ?? (shop.isActive ? "active" : "inactive"),
      totalEarning,
      totalOrders,
      activeOrders,
      totalCustomers,
  ...
  const fetchShops = async () => {
    try {
      setLoading(true);
      const data = await AdminService.getAllShopsDetails(
  ...
  const filterAndSortShops = () => {
    ...
    const filtered = shops
      .filter((shop) => {
        const derived = deriveShopData(shop);
        return (
          derived.name.toLowerCase().includes(term) ||
          derived.location.toLowerCase().includes(term) ||
          (shop.user?.name || "").toLowerCase().includes(term)
        );
      })
      .sort((a, b) => {
        const aDerived = deriveShopData(a);
        const bDerived = deriveShopData(b);

        if (sortBy === "earnings") {
          return bDerived.totalEarning - aDerived.totalEarning;
        } else if (sortBy === "orders") {
          return bDerived.totalOrders - aDerived.totalOrders;
        } else if (sortBy === "rating") {
          return bDerived.rating - aDerived.rating;
        }
        return 0;
      });
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `₹${shops
        .reduce((acc, s) => acc + deriveShopData(s).totalEarning, 0)
        .toLocaleString()}`,
      icon: DollarSign,
    ...
    {
      title: "Total Orders",
      value: shops.reduce((acc, s) => acc + deriveShopData(s).totalOrders, 0),
      icon: Package,
    ...
    {
      title: "Active Orders",
      value: shops.reduce((acc, s) => acc + deriveShopData(s).activeOrders, 0),
      icon: TrendingUp,
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-1">
                          <MapPin size={12} /> {derived.location}
                        </p>
                      </div>
                      <div className="flex items-center gap-1 bg-yellow-100 dark:bg-yellow-900/30 px-2.5 py-1 rounded-lg">
                        ...
                        <span className="text-xs font-bold text-yellow-700 dark:text-yellow-400">
                          {derived.rating ? derived.rating.toFixed(1) : "-"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mb-3">
                      <span
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${derived.status === "active"
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-green-700 dark:text-green-300">
                          ₹{derived.totalEarning.toLocaleString()}
                        </p>
                      </div>
