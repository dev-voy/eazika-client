...
export default function ShopRegistrationPage() {
    ...
    const updateNestedForm = (section: 'bankDetail' | 'documents', field: string, value: any) => {
        setFormData(prev => ({
            ...prev,
    ...
    const validateStep = (step: number) => {
        switch (step) {
            ...
            case 2: 
                ...
                return true;
            case 3: 
                const { accountHolderName, accountNumber, ifscCode, bankName, bankPassbookImage } = formData.bankDetail;
                if (!accountHolderName || !accountNumber || !ifscCode || !bankName || !bankPassbookImage) {
                    alert("Please fill all bank details and upload your passbook/cheque.");
                    return false;
                }
                return true;
            default:
        ...
        return (
            <div className="space-y-1.5">
                ...
                >
                    {displayValue ? (
                        <>
                            <Image src={displayValue} alt="Uploaded" layout="fill" objectFit="cover" className="rounded-lg opacity-90" />
                            <button 
                        ...
                        <>
                            {uploading ? <Loader2 className="animate-spin text-yellow-500" size={16} /> : <Upload className="text-gray-400 mb-1" size={16} />}
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden w-full border border-gray-100 dark:border-gray-700 flex flex-col h-[600px] md:h-[650px]">
            ...
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shrink-0">
                <button onClick={handleBack} className="p-1.5 -ml-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    <ArrowLeft size={20} className="text-gray-700 dark:text-white" />
                </button>
                <div className="flex-1">
                    <h1 className="text-base font-bold text-gray-900 dark:text-white leading-tight">Register Shop</h1>
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">Step {currentStep} of 4</p>
                </div>
            </div>
                    ...
                    {STEPS.map((step) => {
                        ...
                        return (
                            <div key={step.id} className="relative z-10 flex flex-col items-center gap-1">
                                ...
                                <span className={`text-[9px] font-bold ${isActive ? 'text-yellow-600 dark:text-yellow-500' : 'text-transparent'}`}>
                                    {step.title}
                                </span>
                            </div>
                        );
                ...
                <AnimatePresence mode="wait">
                    ...
                    {currentStep === 1 && (
                        ...
                        >
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="relative">
                                        <select 
                                            ...
                                            onChange={(e) => updateForm('shopCategory', e.target.value)}
                            ...
                            <ImageUploader 
                                label="Cover Image" 
                                value={formData.shopImage[0] || ''} 
                                onUpload={(url) => updateForm('shopImage', [url])} 
                            />
                        </motion.div>
                    )}
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1">
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => updateNestedForm('bankDetail', 'ifscCode', e.target.value)}
                                        />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                                    By submitting, you agree to Eazika's Partner Terms & Conditions.
                                </p>
                            </div>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl text-left border border-gray-100 dark:border-gray-700 text-xs space-y-2">
                                <div className="flex justify-between"><span className="text-gray-500">Shop</span> <span className="font-bold dark:text-white">{formData.shopName || "N/A"}</span></div>
                                <div className="flex justify-between"><span className="text-gray-500">Category</span> <span className="font-bold dark:text-white capitalize">{formData.shopCategory}</span></div>
                                <div className="flex justify-between"><span className="text-gray-500">Documents</span> <span className="font-bold text-green-600">4 Uploaded</span></div>
                                <div className="flex justify-between"><span className="text-gray-500">Bank Account</span> <span className="font-bold dark:text-white">Ending in {formData.bankDetail.accountNumber.slice(-4) || 'XXXX'}</span></div>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-700 shrink-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
                {currentStep === 4 ? (
                    ...
                    >
                        {isSubmitting ? <Loader2 className="animate-spin w-4 h-4" /> : <><CheckCircle className="w-4 h-4" /> Submit Application</>}
                    </button>
                ) : (
                    <button 
                        onClick={handleNext}
